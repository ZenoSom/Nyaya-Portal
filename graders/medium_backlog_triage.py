"""
Grader for medium_backlog_triage task.
Task: Prioritize the most urgent criminal backlog.
Expected: case_id=20 (Ananya Panday, Criminal, 550 pending days)
...
def grade(output: str, expected_output: str = "20") -> float:
    """
    Grade the medium_backlog_triage task.
    ...
    try:
        ...
        if isinstance(parsed, dict):
            ...
            if case_id == expected_str:
                return 0.80
            # Partial credit: selected a high-severity criminal case (case_id 1 or 5)
            if case_id in ("1", "5"):
                return 0.40
